...
After these, we decided to work on the steps of the Keccak function $theta$(Theta),$rho$+$pi$(Rho+Pi),$chi$(Chi), and $iota$(Iota). What we did first was try to translate it differently from reference C code but it failed badly so we ended up just using the code and directly translating it to Java and that was successful. We set the rounds to our constant in the instance field "KECCAK_ROUNDS" for the desired 24 rounds. We needed to implement a helper method used in Rho and Theta, as well as the rotate offset for Rho and the Pi lane permutations. In Main, it parses the security bits size (128,224,256,384,512), reads contents of the file into a byte array then goes through our sponge and prints the hashed result. By testing the code, we were comparing the outputs from our code to the outputs of the C code. Eventually we got the same output as the C code.
#show link: underline
Source: https://github.com/mjossaarinen/tiny_sha3/tree/master
